...
if (!function_exists('render_petition_card_block')) {

	/**
	 * Render Petition Card Block
	 *
	 ...
	 *
	 */
	function render_petition_card_block($attributes, $content, $block): string {
		...
		if ($is_custom) {
			...
			} else if ( empty($attributes['postId']) && isset($GLOBALS['post']) ) {
				$post = $GLOBALS['post'];
			}
		...
		ob_start();
		$template_path = locate_template('partials/petition-card.php');
		if ($template_path) {
			extract($args);
			include $template_path;
		} else {
			error_log('❌ Template "partials/petition-card.php" introuvable');
		}
		return ob_get_clean();
	}
}
