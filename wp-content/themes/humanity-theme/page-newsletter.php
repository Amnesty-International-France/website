...
$public_url = sprintf('%s/newsletter?newsletter_id=%s', get_home_url(), $newsletter_id);
?>
	...
	<tr>
		<td align="center" valign="top">
			<table width="580px" align="center" cellPadding="0" cellSpacing="0" border="0" valign="top">
				...
				<tr>
					<td>
						<?php foreach ($articles as $article): $post = $article['lien'];
						    setup_postdata($post);
							...
							<table align="center" style="border:1px solid #d6d6d6;box-shadow:0 5px 25px 0 #e2e2e2;margin-top:20px;margin-bottom:30px" cellPadding="0" cellSpacing="0" border="0" valign="top">
								<tbody>
								<tr>
									<td><a href="<?php echo $post_url; ?>" target="_blank" rel="noopener noreferrer" style="text-decoration:none"><img alt="<?php echo get_the_post_thumbnail_caption($post->ID) ?>" src="<?php echo esc_url(get_the_post_thumbnail_url($post->ID, 'medium')); ?>" width="580" height="295" style="display:block;outline:none;border:none;text-decoration:none"/></a></td>
								</tr>
								<tr>
									<td>
										<table width="100%" cellPadding="10" style="padding:0 10px" cellSpacing="0" border="0" align="left" valign="top">
											...
											<div style="font-size:20px;height:20px;line-height:20px;mso-line-height-rule:exactly"></div>
											<tr>
												<td><a href="<?php echo $post_url; ?>" target="_blank" rel="noopener noreferrer" style="text-decoration:none"><span style="font-family:Oswald, Arial, sans-serif;font-size:24px;line-height:30px;color:#000;font-style:normal;text-transform:uppercase;text-align:left"><?php echo esc_html(get_the_title($post->ID)); ?></span></a></td>
											</tr>
											<tr>
												<td>
...
if (is_admin()):
    ?>
	<button id="copy-source" data-public-url="<?php echo $public_url; ?>" style="position: absolute; top: 3vh; right: 3vw; border: 1px solid #000; padding: 8px 12px; background: transparent; cursor: pointer;">
		Copier
	</button>
	<script>
