...
<script>
	document.addEventListener("DOMContentLoaded", function () {
		...
		input.addEventListener("keyup", function () {
			...
			fetch(`/wp-json/humanity/v1/search-document-democratic?term=${encodeURIComponent(searchTerm)}`)
				.then(res => res.json())
				.then(data => {
					results.innerHTML = "";
					data.forEach(item => {
