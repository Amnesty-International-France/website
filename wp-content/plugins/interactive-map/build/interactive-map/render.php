...
if (!empty($attributes['geoJsonUrl'])) {
    ...
    if ($is_absolute_url) {
        ...
        // Assurez-vous que INTERACTIVE_MAP_URL est défini si vous l'utilisez
        $full_geojson_url = defined('INTERACTIVE_MAP_URL') ? INTERACTIVE_MAP_URL . ltrim($geojson_source, '/') : $geojson_source;
    }
}

// Endpoint de l'API WordPress pour la recherche de structures locales (celui que vous avez déjà)
$custom_local_structures_api_endpoint = rest_url( 'amnesty/v1/local-structures-search' );

// Endpoint du proxy de géocodage que nous venons de créer en PHP
...
>
    ...
    <div class="interactive-map__wrapper">
        <!-- <div class="interactive-map__search-container">
            ...
            <input type="text" id="map-search-input" placeholder="Entrez une ville, un département ou une région..." class="interactive-map__search-input">
            <button type="button" id="map-search-button" class="interactive-map__search-button">Rechercher</button>
