...
// List of predefined countries with all their information
...
// Predefined tile options to simplify user choice
const TILE_LAYER_OPTIONS = [
	{ label: 'CartoDB Voyager', value: 'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png' },
	{ label: 'OpenStreetMap', value: 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png' },
	{ label: 'Stadia Stamen Toner', value: 'https://tiles.stadiamaps.com/tiles/stamen_toner/{z}/{x}/{y}{r}.png' },
];
...
export default function Edit({ attributes, setAttributes }) {
	...
	// --- Functions for manipulating thumbnails ---

	// Updates a specific property (e.g.: 'label', 'lat') of a thumbnail at a certain index
	...
	// Adds a new empty thumbnail to the table
	const addVignette = () => {
		const newVignettes = [...vignettes, {
	...
	// Deletes a thumbnail at a given index
	...
	return (
		<>
			{/* Start of the control panel in the editor sidebar */}
			<InspectorControls>
				...
				<PanelBody title={__('Data & API', 'interactive-map')}>
					<TextControl
						...
						help={__('The endpoint that receives coordinates and returns markers.', 'interactive-map')}
					/>
				</PanelBody>
				...
				<PanelBody title={__('Vignettes', 'interactive-map')} initialOpen={false}>
					<ToggleControl
						label={__('Show Vignettes', 'interactive-map')}
						checked={showVignettes}
						onChange={(value) => setAttributes({ showVignettes: value })}
					/>
					{/* The thumbnail editor is only displayed if the option is enabled */}
					{showVignettes && (
								...
								<div key={index} style={{ border: '1px solid #ccc', padding: '10px', marginBottom: '10px' }}>
									...
									<div style={{ display: 'flex', gap: '10px' }}>
										<TextControl
											...
											type="number"
											value={vignette.lat}
											onChange={(value) => updateVignette(index, 'lat', parseFloat(value))}
										/>
										<TextControl
											label={__('Lng', 'interactive-map')}
											type="number"
											value={vignette.lng}
											onChange={(value) => updateVignette(index, 'lng', parseFloat(value))}
										/>
										<TextControl
											label={__('Zoom', 'interactive-map')}
											type="number"
											value={vignette.zoom}
											onChange={(value) => updateVignette(index, 'zoom', parseFloat(value))}
										/>
									</div>
									<TextareaControl
