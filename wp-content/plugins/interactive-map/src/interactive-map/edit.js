...
export default function Edit( { attributes, setAttributes } ) {
	...
	const onCountryChange = ( slug ) => {
		const selectedCountry = COUNTRY_PRESETS.find(
			( c ) => c.value === slug
		);
		if ( selectedCountry ) {
			setAttributes( {
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Map Settings', 'interactive-map' ) }>
					...
					<SelectControl
						...
						onChange={ ( value ) =>
							setAttributes( { tileLayerUrl: value } )
						}
					/>
				</PanelBody>
				...
				<PanelBody title={ __( 'Appearance', 'interactive-map' ) }>
					<TextControl
						...
						onChange={ ( value ) =>
							setAttributes( { mapBackgroundColor: value } )
						}
					/>
					<TextControl
						label={ __(
						...
						onChange={ ( value ) =>
							setAttributes( { defaultPathColor: value } )
						}
					/>
					<TextControl
						label={ __(
							'Hover Department Color',
							'interactive-map'
						) }
						value={ hoverPathColor }
						onChange={ ( value ) =>
							setAttributes( { hoverPathColor: value } )
						}
					/>
				</PanelBody>
				...
				>
					<ToggleControl
						...
						onChange={ ( value ) =>
							setAttributes( { showVignettes: value } )
						}
					/>
					{ showVignettes && (
			...
			<div { ...blockProps }>
				...
				<p style={ { fontSize: '12px', fontStyle: 'italic' } }>
					{ __(
						'Configure the map in the sidebar.',
						'interactive-map'
					) }
				</p>
			</div>
